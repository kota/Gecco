...
public class Spotlight {
    public enum Shape {
        case Oval(center: CGPoint, diameter: CGFloat)
    ...
    convenience public init(shape: Shape?=nil, text: String) {
        self.init()
        if let shape = shape {
            self.shape = shape
        }
    ...
    public var shape: Shape = .Oval(center: CGPointZero, diameter: 100)
